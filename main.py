...
# ═══════════════════════════════════════════════════════════════════════════════
# API ENDPOINT: Batch Price Lookup (for Portfolio P/L)
# ═══════════════════════════════════════════════════════════════════════════════
# Lightweight endpoint — only returns current price + daily change for each code.
# Significantly faster than /api/score/ — no indicator computation.
# Used by the demo Portfolio tab to display real-time P/L.
# ═══════════════════════════════════════════════════════════════════════════════

@app.get("/api/prices")
async def prices_api(kodes: str = Query(..., description="Kodes saham comma-separated, e.g. BBCA,BBRI")):
    """Batch current prices for portfolio valuation — fast, no scoring."""
    tickers = [k.strip().upper() for k in kodes.split(",") if k.strip()]
    if not tickers:
        return {"ok": True, "data": {}}
    ...
    with ThreadPoolExecutor(6) as pool:
        ...
        for code in tickers:
            futures[pool.submit(
                lambda c=code: yf.download(f"{c}.JK", period="5d", progress=False, auto_adjust=True)
            )] = code
        for f in as_completed(futures):
            code = futures[f]
            try:
                ...
                latest = safe(close[-1])
                prev = safe(close[-2])
                change = safe((latest / prev - 1) * 100) if prev > 0 else 0
                results[code] = {
                    "kode": code,
                    "harga": round(latest),
